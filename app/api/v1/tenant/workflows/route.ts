...
export async function POST(request: NextRequest) {
  ...
  if (!session.isPlatformStaff && !session.roleCodes?.some((r) => ['OWNER', 'ADMIN'].includes(r))) {
    return apiError('Forbidden: Admin role required to create workflow definitions', 'FORBIDDEN', 403);
  }

  try {
    const body = await request.json();
    const { title, entityName, states, transitions } = body;

    if (!title || !entityName || !states || !transitions) {
      return apiError('title, entityName, states, and transitions are required', 'VALIDATION_ERROR', 422);
    }
    ...
    return apiError('Failed to create workflow definition', 'INTERNAL_ERROR', 500);
  }
}
