    ...
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-surface-900 dark:text-surface-100 flex items-center gap-2">
            <Building2 className="w-5 h-5 text-brand-500" /> Department Management
          </h1>
          <p className="text-xs text-surface-500">Configure functional departments and organizational teams</p>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
          {loading ? (
            <div className="text-center p-8 text-xs text-surface-500">Loading departments...</div>
          ) : departments.length === 0 ? (
            <div className="text-center p-8 space-y-2">
              <p className="text-sm font-semibold text-surface-700 dark:text-surface-300">No Departments Configured</p>
              <p className="text-xs text-surface-500">Add departments to structure your organization.</p>
            </div>
          ) : (
            <Table
              columns={[
                ...
                },
                {
                  ...
                  cell: (row) => <span className="text-xs text-surface-500">{new Date(row.createdAt).toLocaleDateString()}</span>,
                },
              ]}
              data={departments}
            />
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-surface-900 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl border border-surface-200 dark:border-surface-800">
            <h3 className="text-lg font-bold text-surface-900 dark:text-surface-100">Add Department</h3>
            <form onSubmit={handleCreate} className="space-y-4">
              <Input
                label="Department Name *"
                placeholder="Engineering / Human Resources / Mathematics"
                value={name}
                onChange={(e) => {
